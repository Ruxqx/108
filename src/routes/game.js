...
async function finishGame(request, game) {
  ...
  await run("UPDATE users SET chips = chips + ? WHERE id = ?", [game.payout, userId]);
  await run(
    `
      INSERT INTO games (
...
router.get("/me", async (request, response, next) => {
  try {
    ...
    const user = await get("SELECT id, username, chips FROM users WHERE id = ?", [request.session.userId]);
...
router.post("/games", async (request, response, next) => {
  try {
    ...
    await refillLoggedInUsersIfNeeded(request.session.userId);
    const user = await get("SELECT chips FROM users WHERE id = ?", [request.session.userId]);
    ...
    const refreshedUser = await get("SELECT id, username, chips FROM users WHERE id = ?", [request.session.userId]);
